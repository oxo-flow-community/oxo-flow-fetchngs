...
Fetch metadata and raw FastQ files from public databases (SRA / ENA / DDBJ /
GEO). Given a list of database identifiers — run accessions (SRR/ERR/DRR),
experiments, studies, biosamples or GEO series — the pipeline retrieves the
ENA run metadata, downloads the FastQ files over FTP, validates every
download against its ENA md5 sum, and auto-creates a samplesheet plus sample
id-mappings (and a MultiQC mappings config) ready for downstream nf-core
pipelines.

## Source

Ported from **[nf-core/fetchngs](https://github.com/nf-core/fetchngs)**,
version `1.12.0` (commit `8ec2d934f9301c818d961b1e4fdf7fc79610bdc5`, MIT
license). This port is maintained independently and **may lag the upstream** —
check the `1.12.0` tag above and the fidelity table below for the exact
ported state.
...
- Docker at runtime, per the environments declared in `main.oxoflow`
  (biocontainers images identical to upstream).
